...
def check_model(model_string, model=None):
    ...
    if model_string not in model_options and model == None:
        raise ValueError(
            "model_string not available in prepped models. Please supply model object."
        )
...
def get_layer_names(layers):
    ...
    for layer in layers:
        ...
        layer_names.append(
            layer_name
            + "-"
            + str(sum(layer_name in string for string in layer_names) + 1)
        )
    return layer_names
...
def remove_duplicate_feature_maps(
    feature_maps, method="hashkey", return_matches=False, use_tqdm=False
):
    ...
    if method == "iterative":
        ...
        target_iterator = (
            tqdm(range(len(layer_names)), leave=False)
            if use_tqdm
            else range(len(layer_names))
        )
        ...
        deduplicated_feature_maps = {
            key: value for (key, value) in feature_maps.items() if key not in matches
        }
    ...
    if method == "hashkey":
        ...
        deduplicated_feature_maps = {
            key: value for (key, value) in feature_maps.items() if key in layers_to_keep
        }
...
def get_feature_maps(
        ...
        if enforce_input_shape:
            ...
            if outputs.shape[0] != inputs.shape[0]:
                ...
                if not check_for_input_axis(outputs, inputs.shape[0]):
                    warning(
                        "Ambiguous input axis in {}. Skipping...".format(module_name)
                    )
                    return None
    ...
    feature_maps = {
        map: features
        for (map, features) in feature_maps.items()
...
def get_empty_feature_maps(
    ...
    empty_feature_maps = get_feature_maps(
        model, inputs, layers_to_retain, remove_duplicates
    )

    for map_key in empty_feature_maps:
        empty_feature_maps[map_key] = torch.empty(
            dataset_size, *empty_feature_maps[map_key].shape[1:]
        )
...
def get_all_feature_maps(
    model,
    inputs,
    layers_to_retain=None,
    remove_duplicates=True,
    include_input_space=False,
    flatten=True,
    numpy=True,
    use_tqdm=True,
):
    ...
    if isinstance(inputs, DataLoader):
        input_size, dataset_size, start_index = (
            inputs.dataset[0].shape,
            len(inputs.dataset),
            0,
        )
        feature_maps = get_empty_feature_maps(
            model,
            next(iter(inputs))[:3],
            input_size,
            dataset_size,
        ...
        for imgs in (
            tqdm(inputs, desc="Feature Extraction (Batch)") if use_tqdm else inputs
        ):
            imgs = imgs.cuda() if next(model.parameters()).is_cuda else imgs
            batch_feature_maps = get_feature_maps(
                model, imgs, layers_to_retain, remove_duplicates=False
            )
            ...
            for map_i, map_key in enumerate(feature_maps):
                feature_maps[map_key][
                    start_index : start_index + imgs.shape[0], ...
                ] = batch_feature_maps[map_key]
        ...
        def hook(module, input, output):
            def process_output(output, module_name):
                    ...
                    if enforce_input_shape:
                        ...
                        if outputs.shape[0] != inputs.shape[0]:
                            ...
                            if not check_for_input_axis(outputs, inputs.shape[0]):
                                feature_maps[module_name] = None
                                warning(
                                    "Ambiguous input axis in {}. Skipping...".format(
                                        module_name
                                    )
                                )
                ...
                if module_name in map_data:
                    ...
                    metadata[module_name][
                        "feature_map_shape"
                    ] = feature_map.numpy().shape[1:]
                    metadata[module_name]["feature_count"] = (
                        feature_map.numpy().reshape(1, -1).shape[1]
                    )
...
def get_feature_map_names(model, inputs=None, remove_duplicates=True):
    feature_map_names = get_empty_feature_maps(
        model, inputs, names_only=True, remove_duplicates=remove_duplicates
    )
