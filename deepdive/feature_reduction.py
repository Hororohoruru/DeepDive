...
def check_reduction_inputs(feature_maps=None, model_inputs=None):
    ...
    if model_inputs is not None and not isinstance(
        model_inputs, (DataLoader, torch.Tensor)
    ):
...
def get_feature_map_filepaths(feature_map_names, output_dir):
    return {
        feature_map_name: os.path.join(output_dir, feature_map_name + ".npy")
...
def recursive_delete_if_empty(path):
    ...
    recurse_list = [
        recursive_delete_if_empty(os.path.join(path, filename))
...
def delete_saved_output(
    output_filepaths, output_dir=None, remove_empty_output_dir=False
):
...
def get_feature_map_srps(
    ...
    if isinstance(feature_maps, np.ndarray) and save_outputs:
        raise ValueError(
            "Please provide a dictionary of the form {feature_map_name: feature_map}"
    ...
    if isinstance(feature_maps, dict) and not save_outputs:
        ...
        for feature_map_name in tqdm(list(feature_maps), desc="SRP Extraction (Layer)"):
            srp_feature_maps[feature_map_name] = get_srps(
                feature_maps[feature_map_name]
            )
    ...
    if isinstance(feature_maps, dict) and save_outputs:
        output_dir = os.path.join(
            output_dir, "_".join(["projections", str(n_projections), "seed", str(seed)])
        )
        ...
        for feature_map_name in tqdm(list(feature_maps), desc="SRP Extraction (Layer)"):
            ...
            if os.path.exists(output_filepath):
                srp_feature_maps[feature_map_name] = np.load(
                    output_filepath, allow_pickle=True
                )
        ...
        if delete_saved_outputs:
            delete_saved_output(
                output_filepaths, output_dir, remove_empty_output_dir=True
            )
...
def srp_extraction(
    ...
    eps=0.1,
    seed=0,
    output_dir="temp_data/srp_arrays",
    delete_saved_outputs=True,
    delete_original_feature_maps=False,
    verbose=False,
):
    ...
    device_name = (
        "CPU" if not torch.cuda.is_available() else torch.cuda.get_device_name()
    )
    ...
    if verbose:
        print(
            "Computing {} SRPs for {}; using {} for feature extraction...".format(
                n_projections, model_string, device_name
            )
        )
...
def get_feature_map_pcs(
    feature_maps,
    n_components=None,
    return_pca_object=False,
    save_outputs=False,
    output_dir="temp_data/pca",
    delete_originals=False,
    delete_saved_outputs=True,
):
    ...
    if isinstance(feature_maps, dict) and save_outputs:
        ...
        for feature_map_name in tqdm(list(feature_maps), desc="PCA Extraction (Layer)"):
            output_filepath = output_filepaths[feature_map_name]
            if not os.path.exists(output_filepath):
                pca_feature_maps[feature_map_name] = get_pca(
                    feature_maps[feature_map_name]
                )
                np.save(output_filepath, srp_feature_maps[feature_map_name])
            if os.path.exists(output_filepath):
                pca_feature_maps[feature_map_name] = np.load(
                    output_filepath, allow_pickle=True
                )
...
def pca_extraction(
    ...
    n_components=None,
    aux_inputs=None,
    aux_feature_maps=None,
    output_dir="temp_data/pca_arrays",
    delete_saved_outputs=True,
    delete_original_feature_maps=False,
    verbose=False,
):
    ...
    if n_components is not None:
        if n_components > n_aux_samples and use_aux_pca:
            raise ValueError(
                "Requesting more components than are available with PCs from auxiliary sample."
            )
        if n_components > n_samples:
            raise ValueError(
                "Requesting more components than are available with stimulus set sample size."
            )
    ...
    pca_printout = (
        "{} Independent PCs"
        if use_aux_pca
        else "up to {} Stimulus PCs".format(n_components)
    )

    if verbose:
        print(
            "Computing {} for {}; using {} for feature extraction...".format(
                pca_printout, model_string, device_name
            )
        )
    ...
    if (feature_maps is None) or (aux_feature_maps is None and use_aux_pca):
        ...
        if aux_feature_maps is None:
            aux_feature_maps = get_all_feature_maps(
                model, aux_inputs, layers_to_retain=list(feature_maps.keys())
            )

    if use_aux_pca:
        pca_args = {
            "feature_maps": aux_feature_maps,
        ...
        if save_outputs:
            raise Warning(
                "save_outputs incompatible with using auxiliary PCA. Ignoring (and not saving)..."
            )
        ...
        for feature_map in feature_maps:
            pca_feature_maps = aux_pcas[feature_map].transform(
...
def rdm_extraction(
    ...
    model_inputs=None,
    feature_maps=None,
    use_torch_corr=False,
    append_filename_suffix=False,
    output_dir="temp_data/rdm",
    delete_saved_outputs=True,
    delete_original_feature_maps=False,
    verbose=True,
):
    ...
    if verbose:
        print(
            "Computing RDMS for {}; using {} for feature extraction...".format(
                model_string, device_name
            )
        )
    ...
    output_file = os.path.join(
        output_dir,
        model_string + "_rdms.pkl" if append_filename_suffix else model_string + ".pkl",
    )
    ...
    if not os.path.exists(output_file):
        if feature_maps is None:
            ...
            feature_maps = get_all_feature_maps(
                model, model_inputs, numpy=not use_torch_corr
            )
