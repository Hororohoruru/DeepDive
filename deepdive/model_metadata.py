...
def get_model_metadata(model_option, convert_to_dataframe=True):
    ...
    feature_counts = [
        layer_metadata[layer]["feature_count"] for layer in layer_metadata
    ]
    parameter_counts = [
        layer_metadata[layer]["parameter_count"] for layer in layer_metadata
    ]
    feature_map_shapes = [
        layer_metadata[layer]["feature_map_shape"] for layer in layer_metadata
    ]
    ...
    if convert_to_dataframe:
        ...
        for layer_index, layer in enumerate(layer_metadata):
            model_metadata_dictlist.append(
                {
                    "model": model_name,
...
if __name__ == "__main__":
    ...
    model_options = {
        **get_model_options(train_type="classification", model_source="torchvision"),
